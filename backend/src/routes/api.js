...
const createApiRoutes = (agenticService, indexingService, contextService, toolingService) => {
    ...
    router.post('/agent/process', async (req, res) => {
        try {
            ...
            logger_1.logger.error('Agent process error', { error: errorMessage });
    ...
    router.post('/index/workspace', async (req, res) => {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error('Index workspace error', { error: errorMessage });
            res.status(500).json({ success: false, error: errorMessage });
        }
    });
    router.post('/index/file', async (req, res) => {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error('Index file error', { error: errorMessage });
            res.status(500).json({ success: false, error: errorMessage });
        }
    });
    router.get('/index/symbols', async (req, res) => {
        try {
            ...
            const symbols = await indexingService.searchSymbols(query);
            res.json({ success: true, data: symbols });
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error('Search symbols error', { error: errorMessage });
            res.status(500).json({ success: false, error: errorMessage });
        }
    });
    // Context routes
    router.post('/context/store', async (req, res) => {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error('Store context error', { error: errorMessage });
            res.status(500).json({ success: false, error: errorMessage });
        }
    });
    router.get('/context/search', async (req, res) => {
        try {
            ...
            const results = await contextService.searchContexts(query);
            const limited = limit ? results.slice(0, parseInt(limit)) : results;
            res.json({ success: true, data: limited });
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error('Search context error', { error: errorMessage });
            res.status(500).json({ success: false, error: errorMessage });
        }
    });
    // Tooling routes
    router.post('/tools/execute', async (req, res) => {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error('Tool execution error', { error: errorMessage });
            res.status(500).json({ success: false, error: errorMessage });
        }
    });
    return router;
